            ...
            words.append(
                {
                    ...
                    "bbox": {
                        ...
                        "width": int(
                            max(point[0] for point in bbox) - min(point[0] for point in bbox)
                        ),
                        "height": int(
    ...
    try:
        ...
        if "properties" in results and "error" not in results["properties"]:
            ...
            summary_parts.append(
                f"Image: {props['dimensions']['width']}x{props['dimensions']['height']} pixels, {props['format']}"
            )
        ...
        if "properties" in results:
            ...
            if "error" not in props:
                output_lines.append(
                    f"📐 Dimensions: {props['dimensions']['width']} x {props['dimensions']['height']}"
                )
                output_lines.append(f"📁 Format: {props['format']} ({props['file_size_mb']} MB)")
                output_lines.append(
                    f"🌈 Colors: {len(props.get('dominant_colors', []))} dominant colors"
                )
    ...
    try:
        ...
        if face_count > 0:
            for i, face in enumerate(result.get("faces", []), 1):
                output_lines.append(
                    f"   Face {i}: position ({face['x']}, {face['y']}) size {face['width']}x{face['height']}"
                )
        ...
        if shape_count > 0:
            for i, shape in enumerate(result.get("shapes", [])[:5], 1):  # Limit to 5 shapes
                output_lines.append(
                    f"   Shape {i}: {shape['vertices']} vertices, area {shape['area']}"
                )
    ...
    parameters={
        ...
        "properties": {
            ...
            "ocr": {
                "type": "boolean",
                "default": True,
                "description": "Enable text extraction (OCR)",
            },
            "ocr_method": {
                "type": "string",
                "default": "auto",
                "description": "OCR method: 'tesseract', 'easyocr', or 'auto'",
            },
            "ocr_languages": {
                "type": "string",
                "default": "eng",
                "description": "OCR languages (e.g., 'eng', 'eng+fra')",
            },
            "detect_objects": {
                "type": "boolean",
                "default": False,
