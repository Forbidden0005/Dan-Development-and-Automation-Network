...
ROLE_PERMISSIONS = {
    ...
    "readonly": {"tools.read", "tools.listdir", "knowledge.recall", "knowledge.list"},
...
class AuthManager:
    ...
    def _setup_audit_logging(self):
        ...
        if not any(
            ...
            and getattr(handler, "baseFilename", None) == audit_base_filename
            for handler in self.audit_logger.handlers
        ):
            audit_handler = logging.FileHandler(audit_log_path)
            audit_handler.setFormatter(
                logging.Formatter("%(asctime)s [AUTH] %(levelname)s: %(message)s")
            )
    ...
    def _save_auth_data(self):
        ...
        try:
            ...
            data = {
                ...
                "sessions": {
                    session_id: {**asdict(session), "permissions": list(session.permissions)}
    ...
    def authenticate(
        ...
        if user.locked_until > current_time:
            self.audit_logger.warning(
                f"Authentication failed - user {user.username} locked until {datetime.fromtimestamp(user.locked_until)}"
            )
            self.auth_attempts.append(attempt)
            return None
    ...
    def get_auth_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_attempts": len(
                [a for a in self.auth_attempts if a.timestamp > current_time - 3600]
            ),
            "failed_attempts": len(
                [
                    a
                    for a in self.auth_attempts
                    if not a.success and a.timestamp > current_time - 3600
                ]
            ),
        }
