        ...
        # Classes + their methods
            ...
            for item in node.body:
                if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    symbols.append(
                        Symbol(item.name, "method", item.lineno, msig, mdoc, parent_class=node.name)
                    )
...
def _extract_js_ts(content: str) -> tuple[list[Symbol], list[FileDep]]:
    ...
    for m in re.finditer(
        r"(?:export\s+)?(?:async\s+)?function\s+(\w+)\s*\(([^)]{0,80})\)", content
    ):
    ...
    for m in re.finditer(
        r"(?:export\s+)?(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s*)?\(([^)]{0,80})\)\s*=>", content
    ):
        name, args = m.group(1), m.group(2).strip()
        line = content[: m.start()].count("\n") + 1
...
def _extract_go(content: str) -> tuple[list[Symbol], list[FileDep]]:
    ...
    for m in re.finditer(
        r"^func\s+(?:\(\w+\s+\*?(\w+)\)\s+)?(\w+)\(([^)]{0,80})\)", content, re.MULTILINE
    ):
...
def _extract_rust(content: str) -> tuple[list[Symbol], list[FileDep]]:
    ...
    for m in re.finditer(
        r"(?:pub\s+)?(?:async\s+)?fn\s+(\w+)\(([^)]{0,80})\)(?:\s*->\s*([^\{]{0,40}))?", content
    ):
...
def _extract_symbols(content: str, ext: str) -> tuple[list[Symbol], list[FileDep]]:
    if ext in (".py", ".pyw"):
...
def _index_project(root_str: str, force: bool = False) -> str:
    ...
    for fp in sorted(root.rglob("*")):
        ...
        row = db.execute("SELECT id, file_hash FROM files WHERE path=?", (str(fp),)).fetchone()
        ...
        with db:
            if row:
                file_id = row["id"]
                db.execute(
                    "UPDATE files SET file_hash=?, line_count=?, indexed_at=? WHERE id=?",
...
def dependency_graph(file_path: str, project_root: str = ".") -> str:
    ...
    row = db.execute(
        "SELECT id, language FROM files WHERE rel_path=? OR path=?", (rel, str(abs_fp))
    ).fetchone()

    if not row:
        db.close()
        return (
            f"'{rel}' is not in the index. Run IndexProject first, " "or check the path is correct."
        )
    ...
    imports = db.execute(
        "SELECT module_name, is_local, from_clause FROM deps WHERE file_id=?", (file_id,)
    ).fetchall()
    ...
    symbols = db.execute(
        "SELECT name, kind, line FROM symbols WHERE file_id=? ORDER BY line", (file_id,)
    ).fetchall()
...
def semantic_search(
    query: str, project_root: str = ".", top_k: int = 10, use_embeddings: bool = True
) -> str:
    """
...
def _fts_search(db: sqlite3.Connection, query: str, top_k: int) -> list[tuple]:
...
def _vector_search(db: sqlite3.Connection, query: str, top_k: int) -> list[tuple]:
