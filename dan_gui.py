...
class DanGUI(ctk.CTk):
    ...
    def _init_dan(self):
        register_core_tools(); register_knowledge_tools(); register_web_tools()
        register_worker_tools(); register_action_tools(); register_skill_tools()
    ...
    def _build_header(self):
        ...
        self._model_badge = ctk.CTkFrame(outer, fg_color=CARD, corner_radius=20,
                                         border_width=1, border_color=BORDER2)
        self._model_badge.grid(row=0, column=1, padx=14, pady=18, sticky="w")
        self._model_lbl = ctk.CTkLabel(self._model_badge,
        ...
        # Keyboard hint
        ctk.CTkLabel(outer, text="⌃N  new   ⌃,  settings   ⌃P  prompts",
                     font=ctk.CTkFont(family="Segoe UI", size=10),
                     text_color=TEXT3).grid(row=0, column=3, padx=12, sticky="e")

        self._status_dot = ctk.CTkLabel(outer, text="●", text_color=SUCCESS,
                                        font=ctk.CTkFont(size=10))
        self._status_dot.grid(row=0, column=4, padx=(0, 4), sticky="e")
        self._status_txt = ctk.CTkLabel(outer, text="Ready",
                                        font=ctk.CTkFont(family="Segoe UI", size=11),
                                        text_color=TEXT3)
        self._status_txt.grid(row=0, column=5, padx=(0, 12), sticky="e")

        _btn(outer, "⚙", self.show_settings, w=38, h=38,
             fg=CARD, hov=CARD_HOV, radius=10
             ).grid(row=0, column=6, padx=(0, 16), pady=12, sticky="e")
    ...
    def _build_sidebar(self):
        ...
        _btn(sb, "＋  New Chat", self.new_chat, h=40,
             fg=PURPLE_DIM, hov=PURPLE, radius=12,
             text_color=TEXT,
             font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold")
             ).grid(row=0, column=0, padx=14, pady=(14, 6), sticky="ew")

        # Search box
        search = ctk.CTkEntry(sb, textvariable=self._search_var,
                              placeholder_text="🔍  Search chats...",
                              fg_color=CARD, border_color=BORDER, border_width=1,
                              text_color=TEXT2, placeholder_text_color=TEXT3,
    ...
    def refresh_sidebar(self):
        ...
        if query:
            sessions = [s for s in sessions
                        if query in self._session_title(s).lower()]
        if not sessions:
            msg = "No matches." if query else "No history yet."
            _label(self.session_list, msg, 12, color=TEXT3
                   ).grid(padx=14, pady=12, sticky="w")
            return
    ...
    def _make_session_item(self, s: dict):
        ...
        frame  = ctk.CTkFrame(self.session_list,
        ...
        ctk.CTkLabel(frame, text="💬", width=22,
                     font=ctk.CTkFont(size=12),
                     text_color=PURPLE if active else TEXT3
                     ).grid(row=0, column=0, padx=(10, 6), pady=8, sticky="w")

        ctk.CTkLabel(frame, text=self._session_title(s), anchor="w",
                     font=ctk.CTkFont(family="Segoe UI", size=12,
                                      weight="bold" if active else "normal"),
                     text_color=TEXT if active else TEXT2, wraplength=170
                     ).grid(row=0, column=1, padx=(0, 4), pady=(8, 2), sticky="ew")
        ctk.CTkLabel(frame, text=self._format_date(s["updated"]), anchor="w",
                     font=ctk.CTkFont(family="Segoe UI", size=10),
                     text_color=PURPLE if active else TEXT3
                     ).grid(row=1, column=1, padx=(0, 4), pady=(0, 8), sticky="w")

        fn = s["filename"]
        def _click(e=None, f=fn): self.load_session(f)
        def _enter(e, fr=frame):
            if not active: fr.configure(fg_color=CARD_HOV)
    ...
    def _build_chat_area(self):
        ...
        self.messages_container = ctk.CTkScrollableFrame(
            self._chat, fg_color=BG, corner_radius=0,
            scrollbar_button_color=BORDER2,
    ...
    def _build_input_area(self):
        ...
        card = ctk.CTkFrame(outer, fg_color=SURFACE2, corner_radius=18,
                            border_width=1, border_color=BORDER2)
        card.grid(row=0, column=0, sticky="ew")
        card.grid_columnconfigure(1, weight=1)

        _btn(card, "📎", self.attach_file, w=34, h=34,
             fg=CARD, hov=CARD_HOV, radius=17
             ).grid(row=0, column=0, padx=(10, 6), pady=10)

        self.input_box = ctk.CTkTextbox(
            card, height=44, fg_color="transparent", border_width=0,
            corner_radius=0,
        ...
        bs = dict(height=30, corner_radius=8, fg_color=CARD, hover_color=CARD_HOV,
        ...
        ctk.CTkOptionMenu(
            tb, variable=self.model_var,
            values=["qwen2.5-coder:14b","qwen2.5-coder:7b","qwen2.5-coder:32b",
                    "llama3.1:8b","deepseek-coder-v2:16b",
                    "claude-sonnet-4-6","claude-opus-4-20250514","gpt-4o","gpt-4o-mini"],
            width=210, height=30, corner_radius=8,
            fg_color=CARD, button_color=BORDER2, button_hover_color=PURPLE_DIM,
            text_color=TEXT2, font=ctk.CTkFont(family="Segoe UI", size=12),
    ...
    def _build_status_bar(self):
        ...
        self._status_lbl = ctk.CTkLabel(
            bar, text=f"  {self.provider_name}  ·  {self.model_name}",
            font=ctk.CTkFont(family="Segoe UI", size=10),
            text_color=TEXT3, anchor="w")
    ...
    def _add_user_message(self, content: str):
        ...
        bubble = ctk.CTkFrame(wrap, fg_color="#1a2f60",
                              corner_radius=14, border_width=1, border_color="#263d7a")
        bubble.grid(row=0, column=0)
        bubble.grid_columnconfigure(0, weight=1)

        tb = ctk.CTkTextbox(
            bubble, fg_color="transparent", border_width=0,
            corner_radius=0, wrap="word",
            font=ctk.CTkFont(family="Segoe UI", size=14),
            text_color="#c8d8ff", activate_scrollbars=False)
        ...
        # Timestamp
        ctk.CTkLabel(bubble, text=ts, anchor="e",
                     font=ctk.CTkFont(family="Segoe UI", size=10),
                     text_color=TEXT3
                     ).grid(row=1, column=0, padx=14, pady=(0, 8), sticky="e")

        # Avatar dot
        ctk.CTkLabel(wrap, text="◉", width=26,
                     font=ctk.CTkFont(size=10), text_color="#3a5aaa"
    ...
    def _add_assistant_message(self, content: str):
        ...
        av = ctk.CTkFrame(outer, width=36, height=36,
        ...
        bubble = ctk.CTkFrame(outer, fg_color=ASST_BG,
        ...
        tb = ctk.CTkTextbox(
            ...
            corner_radius=0, wrap="word",
            font=ctk.CTkFont(family="Segoe UI", size=14),
            text_color=TEXT, activate_scrollbars=False)
        tb.grid(row=0, column=0, padx=14, pady=(10, 6), sticky="ew")
        tb.insert("1.0", content)
        tb.configure(state="disabled")
        lines = int(tb.index("end-1c").split(".")[0])
        ...
        ctk.CTkLabel(bubble, text=ts, anchor="w",
                     font=ctk.CTkFont(family="Segoe UI", size=10),
                     text_color=TEXT3
    ...
    def _add_error_message(self, content: str):
        ...
        bubble = ctk.CTkFrame(outer, fg_color="#1f0a12",
                              corner_radius=12, border_width=1, border_color="#5a1525")
        bubble.grid(row=0, column=0, sticky="ew")

        ctk.CTkLabel(bubble, text=f"⚠  {content}", anchor="w",
                     font=ctk.CTkFont(family="Segoe UI", size=13),
                     text_color="#fb7185", wraplength=700
    ...
    def _attach_copy_hover(self, bubble, tb):
        ...
        cb = ctk.CTkButton(bubble, text="⎘ Copy", width=64, height=24,
        ...
        def _show(e): cb.place(relx=1.0, rely=0.0, anchor="ne", x=-8, y=6)
        def _hide(e): cb.place_forget()
        ...
        def process():
            try:
                response, self.messages = run_agent_loop(
                    text, self.messages, self.provider,
    ...
    def _scroll_to_bottom(self):
        try: self.messages_container._parent_canvas.yview_moveto(1.0)
        except Exception: pass
    ...
    def attach_file(self):
        ...
        if fn: self.input_box.insert("end", f"\n[Attached: {Path(fn).name}]")
    ...
    def show_settings(self):
        ...
        def _sec(r, t):
            _label(scroll, t, 10, "bold", color=TEXT3, anchor="w"
                   ).grid(row=r, column=0, pady=(16,3), **S)

        def _ent(r, var, **kw):
            ctk.CTkEntry(scroll, textvariable=var, fg_color=CARD, border_color=BORDER2,
                         text_color=TEXT, border_width=1,
                         font=ctk.CTkFont(family="Segoe UI", size=13), **kw
                         ).grid(row=r, column=0, pady=(0,4), **S)

        _sec(1, "PROVIDER")
        pv = tk.StringVar(value=cfg.get("provider","ollama"))
        ctk.CTkOptionMenu(scroll, variable=pv,
                          values=["ollama","anthropic","openai","venice"],
                          fg_color=CARD, button_color=BORDER2,
                          button_hover_color=PURPLE_DIM, text_color=TEXT,
                          font=ctk.CTkFont(family="Segoe UI", size=13),
                          corner_radius=8).grid(row=2, column=0, pady=(0,4), **S)
        ...
        sv = tk.StringVar()
        ctk.CTkLabel(bot, textvariable=sv, text_color=SUCCESS,
        ...
        ctk.CTkButton(bot, text="Save Settings", height=40,
                      fg_color=PURPLE, hover_color=PURPLE_HOV,
                      corner_radius=10, text_color=TEXT,
                      font=ctk.CTkFont(family="Segoe UI", size=14, weight="bold"),
                      command=_save).grid(row=1, column=0, padx=24, pady=(8,16), sticky="ew")
    ...
    def show_actions(self):
        ...
        _label(win, "Available Actions", 17, "bold"
               ).grid(row=0, column=0, padx=24, pady=(22,12), sticky="w")
        tb = ctk.CTkTextbox(win, fg_color=CARD, border_color=BORDER2,
    ...
    def show_prompts(self):
        ...
        ctk.CTkButton(left, text="＋  New Prompt", height=36,
                      fg_color=PURPLE_DIM, hover_color=PURPLE, corner_radius=10,
                      text_color=TEXT, font=ctk.CTkFont(family="Segoe UI", size=13),
                      command=lambda: _load(None)
                      ).grid(row=0, column=0, padx=10, pady=12, sticky="ew")

        pl = ctk.CTkScrollableFrame(left, fg_color="transparent",
                                    scrollbar_button_color=BORDER2)
        ...
        _label(nf, "Name", 10, "bold", color=TEXT3, anchor="w"
               ).grid(row=0, column=0, padx=(0,10))
        nv = tk.StringVar()
        ctk.CTkEntry(nf, textvariable=nv, placeholder_text="Prompt name...",
                     fg_color=CARD, border_color=BORDER2, border_width=1,
                     text_color=TEXT, font=ctk.CTkFont(family="Segoe UI", size=13)
                     ).grid(row=0, column=1, sticky="ew")

        editor = ctk.CTkTextbox(
            right, fg_color=CARD, border_width=1, border_color=BORDER2,
            corner_radius=12, text_color=TEXT,
        ...
        def _refresh():
            ...
            if not files:
                _label(pl, "No prompts yet.", 12, color=TEXT3
                       ).grid(padx=10, pady=10, sticky="w")
                return
            for fp in files:
                ctk.CTkButton(
                    pl, text=fp.stem, anchor="w", height=34,
                    fg_color=CARD if fp==_cur[0] else "transparent",
                    hover_color=CARD_HOV, corner_radius=8,
        ...
        def _del():
            if _cur[0] and _cur[0].exists(): _cur[0].unlink()
            _load(None)
        ...
        ctk.CTkButton(br, text="Save", width=80, fg_color=CARD,
                      hover_color=CARD_HOV, text_color=TEXT, **bs2,
                      command=_save).grid(row=0, column=0, padx=(0,8))
        ctk.CTkButton(br, text="Delete", width=80,
                      fg_color="#3f1515", hover_color="#5a1f1f",
                      text_color="#ffaaaa", **bs2,
                      command=_del).grid(row=0, column=1, padx=(0,8))
        ctk.CTkButton(br, text="Use Prompt  ▶", width=150,
                      fg_color=PURPLE_DIM, hover_color=PURPLE,
                      text_color=TEXT, **bs2, command=_use).grid(row=0, column=2)
